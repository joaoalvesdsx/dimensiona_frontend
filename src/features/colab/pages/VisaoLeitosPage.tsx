...
import { getUnidadeById, getSessoesAtivasByUnidadeId, UnidadeInternacao, Leito, SessaoAtiva, StatusLeito, admitirPaciente, liberarSessao } from '@/lib/api';
import { useAuth } from '@/contexts/AuthContext';
import { Bed, User, CheckCircle, Star, LogOut } from 'lucide-react';

const getStatusInfo = (status: StatusLeito) => {
    switch(status) {
        case StatusLeito.ATIVO: return { class: 'bg-red-100 border-red-400 text-red-700', text: 'Ocupado' };
        case StatusLeito.VAGO: return { class: 'bg-green-100 border-green-400 text-green-700', text: 'Vago' };
        case StatusLeito.PENDENTE: return { class: 'bg-yellow-100 border-yellow-400 text-yellow-700', text: 'Pendente' };
        case StatusLeito.INATIVO: return { class: 'bg-gray-200 border-gray-400 text-gray-600 cursor-not-allowed', text: 'Inativo' };
        default: return { class: 'bg-gray-100 border-gray-300', text: 'Desconhecido' };
    }
};
...
export default function VisaoLeitosPage() {
    ...
    const handleAdmitir = async (e: FormEvent) => {
        e.preventDefault();
        if (!unidadeId || !leitoSelecionado || !user?.id || !unidade?.scpMetodoKey || prontuario.length < 3) {
    ...
    const handleDarAlta = async (sessaoId: string) => {
        if (window.confirm('Tem certeza que deseja dar alta para esta internação?')) {
            try {
        ...
        <div className="space-y-6 p-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                {unidade.leitos?.map(leito => {
                    ...
                    const isClickable = statusAtual === StatusLeito.VAGO || statusAtual === StatusLeito.PENDENTE;

                    return (
                        <div key={leito.id} className="relative">
                            <div
                                onClick={() => isClickable && handleLeitoClick(leito.id, statusAtual)}
                                className={`w-full p-4 border-2 rounded-lg flex flex-col items-center justify-center transition-all ${statusInfo.class} ${isClickable ? 'cursor-pointer hover:shadow-lg hover:border-secondary' : ''}`}
                            >
                                <Bed size={32} />
                                <p className="mt-2 font-bold text-lg">{leito.numero}</p>
                                <p className="text-xs uppercase font-semibold">{statusInfo.text}</p>
                                {sessao && (
                            ...
                            {sessao && (
                                <div className="mt-2 space-y-2">
                                    <button onClick={() => navigate(`/unidade/${unidadeId}/sessao/${sessao.id}/avaliar`)} className="w-full flex justify-center items-center gap-2 px-3 py-1.5 text-sm text-white bg-blue-600 rounded-md hover:bg-blue-700">
                                        <Star size={16} /> Avaliar
                                    </button>
                                    <button onClick={() => handleDarAlta(sessao.id)} className="w-full flex justify-center items-center gap-2 px-3 py-1.5 text-sm text-white bg-orange-500 rounded-md hover:bg-orange-600">
                            ...
                            {leitoSelecionado === leito.id && (
                                <form onSubmit={handleAdmitir} className="mt-2 p-3 bg-white border rounded-md shadow-sm animate-fade-in-down">
                                    <label htmlFor="prontuario" className="text-xs font-medium text-gray-700">Nº do Prontuário</label>
                                    <input
                                        id="prontuario"
                                        type="text"
                                        value={prontuario}
                                    ...
                                    />
                                    <button type="submit" className="mt-2 w-full flex justify-center items-center gap-2 px-3 py-1.5 text-sm text-white bg-green-600 rounded-md hover:bg-green-700">

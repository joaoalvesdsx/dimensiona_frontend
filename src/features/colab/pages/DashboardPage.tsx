...
import { getHospitalStats, getBaselinesByHospitalId, HospitalStats, Baseline } from '@/lib/api';
...
import { ChartData, HeatMapData, WaterfallData } from '@/features/admin-hospital/types/hospital';
...
export default function HospitalDashboardPage() {
    ...
    if (!stats) return <div>Não foram encontrados dados estatísticos para este hospital.</div>;
    ...
    const heatMapData: HeatMapData[] = stats.unidades.map(u => ({
        ...
        status: u.ocupacao.taxaOcupacao >= 0.8 ? 'high' : u.ocupacao.taxaOcupacao >= 0.6 ? 'medium' : 'low'
    }));
    ...
    const waterfallData: WaterfallData[] = baselines.flatMap(b => 
        b.setores.map((setor, index, arr) => ({
            ...
            cumulative: arr.slice(0, index + 1).reduce((acc, s) => acc + (parseFloat(s.custo) || 0), 0),
            type: 'sector' as const
        }))
    );
    if (baselines.length > 0) {
        const totalCost = parseFloat(baselines[0].custo_total) || 0;
        waterfallData.unshift({ name: 'Custo Total Baseline', value: totalCost, cumulative: totalCost, type: 'total' });
    }
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-3xl font-bold text-primary">Dashboard do Hospital</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <MetricCard title="Taxa de Ocupação Média" value={`${occupationMetrics.avgOccupation}%`} icon={BarChart} />
                <MetricCard title="Total de Leitos" value={stats.totalLeitos} icon={Building} />
                <MetricCard title="Custo Total (Baseline)" value={`R$ ${(parseFloat(baselines[0]?.custo_total) / 1000 || 0).toFixed(1)}k`} icon={DollarSign} />
                <MetricCard title="Total de Colaboradores" value={baselines[0]?.quantidade_funcionarios || 0} icon={Users} />
            </div>
            ...
            <div className="space-y-6">
                ...
                {baselines.length > 0 && (
                    <WaterfallChart data={waterfallData} title="Análise de Custos (Baseline)" />
                )}

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <PizzaChart data={pizzaData} title="Distribuição de Leitos por Unidade" />

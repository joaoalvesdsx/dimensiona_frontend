...
export default function SitiosFuncionaisAdminPage() {
    const { setorId: unidadeId, hospitalId } = useParams<{ setorId: string, hospitalId: string }>();
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            if (editingSitio) {
                // Edição
                await updateSitioFuncional(editingSitio.id, { nome: formData.nome, descricao: formData.descricao });
            } else {
                // Criação
            ...
            setError(editingSitio ? 'Falha ao atualizar o sítio.' : 'Falha ao criar o sítio.');
        }
    };

    const handleDelete = async (sitioId: string) => {
        if (window.confirm('Tem certeza que deseja excluir este sítio funcional? Todas as alocações de cargos serão perdidas.')) {
            try {
            ...
            {isFormVisible && (
                <div className="bg-white p-6 rounded-lg border animate-fade-in-down">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <h3 className="text-lg font-semibold text-primary">{editingSitio ? 'Editar' : 'Adicionar Novo'} Sítio Funcional</h3>
                        <div>
                            ...
                            {unidade?.sitiosFuncionais?.map((sitio) => (
                                <tr key={sitio.id}>
                                    ...
                                    <td className="px-4 py-2 whitespace-nowrap text-right text-sm space-x-2">
                                        <button onClick={() => setManagingSitio(sitio)} className="text-green-600 hover:text-green-800" title="Gerenciar Cargos">
                                            <Users size={18} />
                                        </button>
                                        <button onClick={() => handleOpenForm(sitio)} className="text-secondary hover:opacity-70" title="Editar Sítio">
                                            <Edit size={18} />
                                        </button>
                                        <button onClick={() => handleDelete(sitio.id)} className="text-red-600 hover:opacity-70" title="Excluir Sítio">
                                            <Trash2 size={18} />
                                        </button>
                                    </td>
                                </tr>

...
import { getLeitosByUnidade, createLeito, updateLeito, deleteLeito, Leito, CreateLeitoDTO, UpdateLeitoDTO } from '@/lib/api';
...
export default function LeitosAdminPage() {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            setError(formData.id ? 'Falha ao atualizar o leito.' : 'Falha ao criar o leito.');
        }
    };
            ...
            {isFormVisible && (
                <div className="bg-white p-6 rounded-lg border animate-fade-in-down">
                    <form onSubmit={handleSubmit}>
                        <h3 className="text-lg font-semibold mb-2 text-primary">{formData.id ? 'Editar Leito' : 'Adicionar Leito'}</h3>
                        <div className="flex items-end gap-4">
                            <div className="flex-grow">
                                <label htmlFor="numero" className="block text-sm font-medium text-gray-700">Número/Nome do Leito</label>
                                <input
                                    type="text"
                                    id="numero"
                            ...
                            <button type="submit" className="px-4 py-2 h-10 text-white bg-green-600 rounded-md hover:bg-green-700">
                                Salvar
                            </button>
                        </div>
                    </form>
                ...
                {!loading && !error && (
                    <table className="min-w-full">
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {leitos.map((leito) => (
                                <tr key={leito.id}>
                                    <td className="px-4 py-2 whitespace-nowrap text-sm font-medium">{leito.numero}</td>
                                    <td className="px-4 py-2 whitespace-nowrap text-sm">{leito.status}</td>
                                    <td className="px-4 py-2 whitespace-nowrap text-right text-sm space-x-2">
                                        <button onClick={() => handleEdit(leito)} className="text-secondary"><Edit size={18} /></button>
                                        <button onClick={() => handleDelete(leito.id)} className="text-red-600"><Trash2 size={18} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>

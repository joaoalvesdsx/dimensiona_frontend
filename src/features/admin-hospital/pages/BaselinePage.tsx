...
import { getBaselinesByHospitalId, createBaseline, updateBaseline, deleteBaseline, Baseline, CreateBaselineDTO, UpdateBaselineDTO, SetorBaseline, getHospitalById, Hospital } from '@/lib/api';
...
export default function BaselinePage() {
  ...
  useEffect(() => {
    const total = formData.setores?.reduce((sum, setor) => sum + parseFloat(String(setor.custo) || '0'), 0) || 0;
  ...
  const addSetor = () => {
      const novosSetores = [...(formData.setores || []), { nome: '', custo: '0', ativo: true }];
      setFormData(prev => ({ ...prev, setores: novosSetores }));
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const setoresAsString = formData.setores?.map(s => JSON.stringify(s)) || [];

    try {
      if (baseline) { // Modo Edição
        const updateData: UpdateBaselineDTO = { ...formData, setores: formData.setores };
      ...
      setError(baseline ? 'Falha ao atualizar o baseline.' : 'Falha ao criar o baseline.');
    }
  };

  const handleDelete = async () => {
    if (!baseline) return;
    if (window.confirm('Tem certeza que deseja excluir este baseline? Esta ação não pode ser desfeita.')) {
      try {
      ...
      <div className="flex justify-between items-center">
        ...
        {baseline && !isFormVisible && (
            <div className='flex gap-2'>
                <button onClick={handleDelete} className="px-4 py-2 text-white bg-destructive rounded-md hover:opacity-90">
                    <Trash2 className="inline-block mr-2 h-4 w-4"/> Excluir
                </button>
                <button onClick={handleEdit} className="px-4 py-2 text-white bg-secondary rounded-md hover:opacity-90">
      ...
      {isFormVisible ? (
        <div className="bg-white p-6 rounded-lg border animate-fade-in-down">
          <form onSubmit={handleSubmit} className="space-y-6">
            <h2 className="text-xl font-semibold text-primary">{baseline ? 'Editar Baseline' : 'Criar Novo Baseline'}</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b pb-6">
               <div>
                  <label className="block text-sm font-medium text-gray-700">Nome do Baseline</label>
                  <input name="nome" value={formData.nome || ''} onChange={handleChange} placeholder="Ex: Baseline 2024" required className="p-2 border rounded-md w-full mt-1"/>
               </div>
               <div>
                  <label className="block text-sm font-medium text-gray-700">Qtd. Total de Funcionários</label>
                  <input name="quantidade_funcionarios" type="number" value={formData.quantidade_funcionarios || ''} onChange={handleChange} placeholder="0" className="p-2 border rounded-md w-full mt-1"/>
               </div>
               <div>
                  <label className="block text-sm font-medium text-gray-700">Custo Total (Calculado)</label>
                  <input value={custoTotalFormatado} readOnly className="p-2 border rounded-md w-full mt-1 bg-slate-100 text-gray-600 font-semibold"/>
               </div>
            </div>
            ...
            <div>
                ...
                <div className="space-y-3">
                    {formData.setores?.map((setor, index) => (
                        <div key={index} className="grid grid-cols-[1fr,1fr,auto,auto] gap-3 items-center p-2 rounded-md hover:bg-slate-50">
                            ...
                            <CurrencyInput value={setor.custo || '0'} onChange={(value) => handleSetorChange(index, 'custo', value)} />
            ...
            <div className="flex justify-end pt-4 border-t gap-2">
              <button type="button" onClick={handleCancel} className="px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300">Cancelar</button>
        ...
        <div className="text-center p-10 border-2 border-dashed rounded-lg">
            ...
            <h3 className="text-md font-semibold text-slate-700">Nenhum baseline cadastrado para este hospital.</h3>
            <p className="text-sm text-slate-500">Clique em "Novo Baseline" para começar.</p>
             <button onClick={() => setIsFormVisible(true)} className="mt-4 px-4 py-2 text-white bg-primary rounded-md hover:opacity-90">
                <PlusCircle className="inline-block mr-2 h-4 w-4"/> Criar Novo Baseline
            </button>
        </div>
      ) : null}
    </div>

...
import { getHospitais, createHospital, updateHospital, deleteHospital, getRegioes, Hospital, CreateHospitalDTO, Regiao } from '@/lib/api';
...
export default function HospitaisPage() {
  ...
  const [formData, setFormData] = useState<Partial<Hospital & { regiaoId?: string }>>(initialFormState);

  const fetchData = async () => {
    setLoading(true);
    setError(null);
    try {
      const [hospitaisData, regioesData] = await Promise.all([getHospitais(), getRegioes()]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(formData.id ? 'Falha ao atualizar o hospital.' : 'Falha ao criar o hospital.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-primary">Gerenciamento de Hospitais</h1>
        <button
      ...
      {isFormVisible && (
        <div className="bg-white p-6 rounded-lg border animate-fade-in-down">
          <form onSubmit={handleSubmit} className="space-y-4">
            <h2 className="text-xl font-semibold mb-4 text-primary">{formData.id ? 'Editar Hospital' : 'Adicionar Novo Hospital'}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input name="nome" value={formData.nome || ''} onChange={handleChange} placeholder="Nome do Hospital" required className="p-2 border rounded-md"/>
              <input name="cnpj" value={formData.cnpj || ''} onChange={handleChange} placeholder="CNPJ" required className="p-2 border rounded-md"/>
              <input name="endereco" value={formData.endereco || ''} onChange={handleChange} placeholder="Endereço" className="p-2 border rounded-md"/>
              <input name="telefone" value={formData.telefone || ''} onChange={handleChange} placeholder="Telefone" className="p-2 border rounded-md"/>
               <select name="regiaoId" value={formData.regiaoId || ''} onChange={handleChange} className="p-2 border rounded-md">
                  ...
                  hospitais.map((hospital) => (
                    <tr key={hospital.id} className="hover:bg-slate-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                        <Link to={`/hospital/${hospital.id}/dashboard`} className="text-primary font-semibold hover:underline">
                          {hospital.nome}
                        </Link>
                      </td>
